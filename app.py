...
def sadtalker_demo(
    ...
    with gr.Blocks(analytics_enabled=False) as sadtalker_interface:
                    ...
                    with gr.TabItem("Settings"):
                        ...
                        with gr.Column(variant="panel"):
                            ...
                            is_still_mode = gr.Checkbox(
                                label="Still Mode (fewer hand motion, works with preprocess `full`)"
                            )
        ...
        def save_mp4_file_and_close(interface):
            generated_mp4_path = os.path.join(
                "mp4", os.path.splitext(driven_audio.value)[0] + ".mp4"
            )
            gen_video.save(generated_mp4_path)
            interface.close()

        interface.onclose(save_mp4_file_and_close)
